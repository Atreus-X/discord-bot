...
class ArmsRaceCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.creds = None
        self.calendar_id = os.environ.get('AR_CALENDAR_ID')
        self.translation_enabled = os.environ.get('TRANSLATE_AR', 'False').lower() in ('true', '1', 't')
    ...
    def load_announced_events(self):
        ...
        if os.path.exists(ANNOUNCED_AR_FILE):
            try:
                with open(ANNOUNCED_AR_FILE, 'r') as f:
    ...
    def save_announced_events(self):
        ...
        with open(ANNOUNCED_AR_FILE, 'w') as f:
    ...
    async def get_events(self, time_min, time_max):
        ...
        if not self.calendar_id:
            logging.error("AR_CALENDAR_ID environment variable is not set.")
            return []
        try:
