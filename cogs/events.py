...
class PaginatorView(discord.ui.View):
    ...
    async def on_timeout(self):
        try:
            ...
            pass # Can't edit message if interaction is gone
...
class EventsCog(commands.Cog):
    ...
    async def upcoming_events(self, ctx: commands.Context):
        ...
        try:
            ...
            pages = []
            current_page = "**The Event Schedule for the Next 3 Days**\n------------------------------------\n\n"
            for event_str in event_strings:
                if len(current_page) + len(event_str) + 2 > 1900: # Leave some room for footer
                    pages.append(current_page)
                    current_page = f"**The Event Schedule for the Next 3 Days (cont.)**\n------------------------------------\n\n" + event_str
                else:
